...
exports.run = async(client, message, args) => {
  ...
  if(!message.member.roles.has(rol)) return message.channel.sendEmbed(new Discord.RichEmbed().setDescription('Yetkiniz Bulunmamakta veya Rol ayarlamadınız .bansistemi ile Ban Sistemini ayarlayıp tekrar deneyiniz')           .setColor("RANDOM"));
 ...
 if(limittt >= sayı) return message.channel.send(`${sayı ? sayı : '0'} Sayıdan Fazla ban atamazsın!`)
  if (!message.guild) {
  ...
  if (message.mentions.users.size < 1) return message.reply('Banlanacak kullanıcıyı etiketlemelisin').catch(console.error);
